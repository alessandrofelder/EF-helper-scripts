setwd("/media/alessandro/Seagate Expansion Drive/vertebra-study-small-step-dave/")
library(ggplot2)
library(dplyr)
library(kSamples)

histo.vertebrae <- read.csv("/media/alessandro/Seagate Expansion Drive/vertebra-study-small-step-dave/clean-histo-parameters-vertebra-small-step-images-EF-combined.csv", stringsAsFactors = FALSE)
histo.vertebrae$age <- c(88,86,24,84,84,84,86,69,32,72,69,60,52,81,65,67,68,85,76,83,69,79)
...
vertebra.data <- t(read.csv("/media/alessandro/Seagate Expansion Drive/vertebra-study-small-step-dave/vertebra-table-combined.csv", header=FALSE, comment.char="#"))
id.map <- read.csv("/media/alessandro/Seagate Expansion Drive/vertebra-study-small-step-dave/id-map.csv", comment.char="#")
setwd("/media/alessandro/Seagate Expansion Drive/vertebra-study-small-step-dave/plots/")

colnames(vertebra.data)<-vertebra.data[1,]
vertebra.data <- vertebra.data[2:23,]
...
standard.parameters <- read.csv("/media/alessandro/Seagate Expansion Drive/vertebra-study-small-step-dave/standard-parameters.csv")
colnames(standard.parameters)[4] <- "SMI+"
colnames(standard.parameters)[5] <- "SMI-"

v <-as.data.frame(vertebra.data)
v$age <- c(88,86,24,84,84,84,86,69,32,72,69,60,52,81,65,67,68,85,76,83,69,79)
...
#Shapiro-tests give p>0.05 for all v$x except x=age
options(scipen=999)
...
test <- cor.test(v$BVTV,v$SMI)

plot(v$BVTV,v$SMI, cex.lab=2.5,cex.axis=2,cex.main=2.5,cex=3, xlab="bone volume fraction", ylab = "SMI", main = substitute(paste("Pearson's r = ",nn,", p = ",n2), list(nn=round(test$estimate,2),n2=round(test$p.value,5))),ylim=c(-1,4))
...
test <- cor.test(v$BVTV,v$thickness)
plot(v$BVTV,v$thickness, pch=19, cex.lab=2.5,cex.axis=2,cex=3,cex.main=2.5,xlab="bone volume fraction", ylab = "Mean trabecular thickness [mm]", main = substitute(paste("Pearson's r = ",nn,", p = ",n2), list(nn=round(test$estimate,2),n2=round(test$p.value,5))))

test <- cor.test(v$BVTV,v$thicknessMax)
plot(v$BVTV,v$thicknessMax, pch=19, cex.lab=2.5,cex.axis=2,cex=3,cex.main=2.5,xlab="bone volume fraction", ylab = "Maximum trabecular thickness [mm]", main = substitute(paste("Pearson's r = ",nn,", p = ",n2), list(nn=round(test$estimate,2),n2=round(test$p.value,5))))

test <- cor.test(v$BVTV,as.numeric(as.character(v$modes)))
dev.off()